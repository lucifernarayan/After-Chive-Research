...
* **Investigator Model**: `openai/gpt-5.6-luna` via OpenRouter OpenAI SDK (`OPENROUTER_API_KEY`).
...
Phase 4A executes the full comparative benchmark across the frozen 15-case dataset (`cases/evaluation_cases.json`) using `openai/gpt-5.6-luna` via OpenRouter as the investigator LLM.

### API Consumption & Rate-Limit Protocol:
- **Investigator Provider**: OpenRouter (`OPENROUTER_API_KEY`)
- **Investigator Model**: `openai/gpt-5.6-luna`
- **Target Model**: `google/gemma-2-2b-it` (PyTorch hooks locally, 0 LLM calls)
- **Expected Requests**: Exactly **45 OpenRouter API requests** for 15 cases (3 calls/case: Agent 1 hypothesis generation, Agent 1 blind prediction, Agent 2 blind prediction).
- **Rate-Limit & Retry Handling**: Bounded exponential backoff on transient errors (HTTP 500, 502, 503, 504) with clean termination on rate limits (HTTP 429) preserving atomic checkpoints.
- **No Model Fallback**: No silent fallback or automatic model switching is performed.
...
### Output Files:
* `results/phase4a_openrouter_luna_real_results.json`: Detailed per-case records (hypotheses, experiment trajectories, predictions, hidden variant outcomes, candidate logprobs, elapsed times).
* `results/phase4a_openrouter_luna_real_summary.json`: Summary metrics report ($\Delta Acc$, family breakdown, contingency breakdown, Brier scores).

### Execution Commands:
```bash
# Run standalone OpenRouter API smoke test (1 test call)
python scripts/smoke_test_openrouter.py --mock   # Mock mode
python scripts/smoke_test_openrouter.py          # Real API call (Requires OPENROUTER_API_KEY)

# Run dry-run mock mode evaluation
python scripts/phase4a_evaluation.py --mock-gemma --mock-gemini

# Run real benchmark evaluation (Requires CUDA GPU, HF_TOKEN, and OPENROUTER_API_KEY)
